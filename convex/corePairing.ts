...
export const registerPairingCore = mutation({
	...
	handler: async (ctx, args) => {
		...
		const value = {
			...
			claimedAt: undefined,
			syncCredentialHash: undefined,
			syncCredentialIssuedAt: undefined,
		}
...
export const claimPairingCore = mutation({
	...
	handler: async (ctx, args) => {
		...
		await ctx.db.patch(pairing._id, {
			...
			ownerUserId: userId,
			claimedAt: now,
			syncCredentialHash: await hashCredential(syncCredential),
			syncCredentialIssuedAt: now,
		})
		return {
...
export const finalizePairingCore = mutation({
	...
	handler: async (ctx, args) => {
		...
		const value = {
			...
			syncedAt: now,
			syncCredentialHash: pairing.syncCredentialHash,
		}
		if (existing) await ctx.db.patch(existing._id, value)
