...
export interface AmberiteAccessUiActivityEntry {
	...
	event: { type: string; label: string; details?: string }
	timestamp: string
}
...
export function coreAccessRoleToUi(
	role: CoreAccessRole,
	preset?: CorePermissionPreset,
...
export function toAmberiteAccessUiMember(member: CoreAccessMember): AmberiteAccessUiMember {
	return {
		...
		user: { id: member.user_id, username: member.display_name ?? member.user_id },
...
export function toAmberiteActivityEntry(
	...
	return {
		...
		world: entry.instance_id
			? { id: entry.instance_id, name: instanceName ?? entry.instance_id }
			: null,
		event: {
			type: entry.action,
