...
import { CoreApiError, CoreNetworkError } from './errors'
import type {
	CoreAccessPatchBody,
	CoreAccessResponse,
...
async function request(ctx: CoreCallContext, url: string, init?: RequestInit): Promise<Response> {
	...
	try {
		...
		throw new CoreNetworkError(reason)
	} finally {
...
async function apiFetch<T>(ctx: CoreCallContext, url: string, init?: RequestInit): Promise<T> {
	...
	try {
		...
		const reason = e instanceof Error ? e.message : String(e)
		throw new CoreNetworkError(`Invalid JSON response from ${url} — ${reason}`)
	}
}
...
export function uploadModFile(ctx: CoreCallContext, id: string, file: File): CoreUploadHandle {
...
export function updateMod(
	...
	return apiFetch(ctx, `${instanceBaseUrl(ctx, id)}/mods/${encodeURIComponent(filename)}/update`, {
		method: 'PUT',
	})
}
...
function xhrUpload(ctx: CoreCallContext, url: string, file: File): CoreUploadHandle {
	...
	xhr.onerror = () => reject(new CoreNetworkError('Network error during upload'))
	xhr.onabort = () => reject(new CoreNetworkError('Upload aborted'))
	xhr.timeout = ctx.timeoutMs ?? DEFAULT_TIMEOUT_MS
	xhr.ontimeout = () => reject(new CoreNetworkError('Upload timed out'))
