//! Profile state, persistence, and content scanning.
//!
//! Key sections: profile and type definitions (lines 33-321), SQL row mapping
//! and selection (lines 324-450), profile CRUD/upsert and launcher migrations
//! (lines 479-894), project/content scanning (lines 980-1268), and project
//! mutation helpers (lines 1271-1455).
    ...
    ($predicate:tt, $param:ident) => {
        sqlx::query_as::<_, ProfileQueryResult>(
            ...
            SELECT
                ...
                override_java_path,
                json(override_extra_launch_args) as override_extra_launch_args, json(override_custom_env_vars) as override_custom_env_vars,
...
impl Profile {
    ...
    pub async fn upsert(
        ...
        let profile_type = self.profile_type.as_str();
...
mod tests {
    ...
    #[tokio::test]
    async fn upsert_preserves_amberite_profile_fields_and_update_channel() {
        let pool = SqlitePool::connect("sqlite::memory:").await.unwrap();
        sqlx::migrate!().run(&pool).await.unwrap();

        for (index, profile_type, preferred_update_channel) in [
            (0, ProfileType::Client, ReleaseChannel::Release),
            (1, ProfileType::Server, ReleaseChannel::Beta),
            (2, ProfileType::Synced, ReleaseChannel::Alpha),
        ] {
            let now = Utc::now();
            let profile = Profile {
                path: format!("profile-{index}"),
                install_stage: ProfileInstallStage::Installed,
                launcher_feature_version: LauncherFeatureVersion::MOST_RECENT,
                profile_type,
                core_instance_id: Some(format!("core-{index}")),
                server_manifest_json: Some(serde_json::json!({ "id": index })),
            ...
            profile.upsert(&pool).await.unwrap();
            let saved = Profile::get(&profile.path, &pool).await.unwrap().unwrap();

            assert_eq!(saved.profile_type, profile.profile_type);
            assert_eq!(saved.core_instance_id, profile.core_instance_id);
            assert_eq!(saved.server_manifest_json, profile.server_manifest_json);
            assert_eq!(
                saved.preferred_update_channel,
                profile.preferred_update_channel
            );
        }
    }
