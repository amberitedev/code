# @amberite/amberite-api — Implementation Plan v2

## What we're building and why

`@amberite/amberite-api` is the single package that handles every bit of external communication
...
Whether `packages/ui` should depend on `@amberite/amberite-api` directly (so shared
...
1. Rename `packages/core-client` to `packages/amberite-api`, update package name to `@amberite/amberite-api`.
