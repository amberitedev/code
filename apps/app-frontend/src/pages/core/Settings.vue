...
onMounted(async () => {
	if (isPreviewConnected.value) {
		hydrate({
			name: 'Amberite Core',
			description: isPreviewLocal.value ? 'Local Core running on this computer.' : 'External Core linked through pairing.',
			banner: '',
		...
		<template v-else>
			<section class="overflow-hidden rounded-[28px] bg-surface-3">
				<div class="flex min-h-36 items-end bg-surface-4 bg-cover bg-center p-6" :style="form.banner ? { backgroundImage: `url(${form.banner})` } : {}">
					<div>
						<div class="mb-2 flex items-center gap-2 text-sm font-bold text-brand"><ServerStackIcon /> {{ isLocal ? 'Local Core' : 'External Core' }}</div>
