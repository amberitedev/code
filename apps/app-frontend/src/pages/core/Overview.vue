...
import { ChartIcon, LinkIcon, PlusIcon, ServerStackIcon, UsersIcon } from '@modrinth/assets'
...
const displayProfiles = computed(() => profiles.value)
		...
		<section v-if="!displayGroup" class="overflow-hidden rounded-[28px] bg-surface-3">
			<div class="grid gap-6 p-6 lg:grid-cols-[1.1fr_0.9fr] lg:p-8">
				<div class="flex flex-col justify-between gap-10">
					...
					<div v-if="displayUser" class="flex items-center gap-3">
						<Avatar :src="displayUser.image" :alt="displayUser.username" size="42px" circle />
						<div>
							<div class="font-bold text-contrast">{{ displayUser.displayName ?? displayUser.username }}</div>
							<div class="text-sm text-secondary">Friend code: {{ displayUser.friendCode ?? 'Unavailable' }}</div>
						</div>
					</div>
				</div>
