...
import { SYNCED_PERMISSION_PRESET_LABELS, type SyncedPermissionPreset } from '@/pages/instance/synced/use-synced-permissions'
...
const { isPreviewConnected, fakeUsers } = useCorePreview()
...
const previewInvites = ref<Awaited<ReturnType<ReturnType<typeof useSocialClient>['searchUsers']>>>([])
const previewPresets = reactive<Record<string, SyncedPermissionPreset>>({})
const displayGroup = computed(() => group.value ?? (isPreviewConnected.value ? { group: { name: 'Amberite Core' } } : null))
const displayMembers = computed(() =>
	members.value.length
		? members.value
		: [
				...fakeUsers.slice(0, 4).map((user, index) => ({
					_id: `preview-member-${user.userId}`,
					userId: user.userId,
					role: index === 0 ? ('owner' as Role) : index === 1 ? ('admin' as Role) : ('member' as Role),
					status: 'active',
					permissionPreset: index === 0 ? 'owner' : index === 1 ? 'admin' : index === 2 ? 'client-only' : 'member',
					user,
				})),
				...previewInvites.value.map((user) => ({
					_id: `preview-invite-${user.userId}`,
					userId: user.userId,
					role: inviteRole.value === 'admin' ? ('admin' as Role) : ('member' as Role),
					status: 'invited',
					permissionPreset: inviteRole.value === 'admin' ? 'admin' : 'member',
					user,
				})),
			],
)
...
const excludedUserIds = computed(() => new Set(displayMembers.value.map((member) => member.userId)))
const fakeSearchResults = computed(() => {
	const q = query.value.trim().toLowerCase()
	return fakeUsers
		.slice(4)
		.filter((user) => !excludedUserIds.value.has(user.userId))
		.filter((user) =>
			q ? `${user.username} ${user.displayName} ${user.friendCode}`.toLowerCase().includes(q) : true,
		)
})
const displayResults = computed(() => [
	...results.value.filter((user) => !excludedUserIds.value.has(user.userId)),
...
function presetOf(member: Member | (typeof displayMembers)['value'][number]): SyncedPermissionPreset {
	if (!group.value && previewPresets[member.userId]) return previewPresets[member.userId]
	if (member.permissionPreset && (presets as string[]).includes(member.permissionPreset)) return member.permissionPreset as SyncedPermissionPreset
	return member.role === 'owner' ? 'owner' : member.role === 'admin' ? 'admin' : 'member'
}
function updatePreset(member: Member | (typeof displayMembers)['value'][number], preset: SyncedPermissionPreset) {
	if (!group.value && isPreviewConnected.value) {
		previewPresets[member.userId] = preset
		return
	}
	void setMemberRole(member.userId, roleForPreset(preset), preset)
}
function roleForPreset(preset: SyncedPermissionPreset): Role {
	return preset === 'admin' ? 'admin' : 'member'
}
function canAct(member: Member | (typeof displayMembers)['value'][number]) { return displayCanManage.value && member.userId !== currentUser.value?.userId && rank(member.role) < myRank.value }
async function search() {
	searching.value = true
	try {
		results.value = query.value.trim()
			? await useSocialClient().searchUsers(query.value.trim()).catch(() => [])
			: []
	} finally {
...
async function invite(userId: string) {
	const fakeUser = fakeUsers.find((user) => user.userId === userId)
	if (fakeUser && !previewInvites.value.some((user) => user.userId === userId)) {
		previewInvites.value.push(fakeUser)
		modal.value?.hide()
		return
	}
	await inviteToGroup({
...
<template>
	<div v-if="displayGroup" class="flex w-full max-w-[min(928px,calc(95vw-10rem))] flex-col gap-5">
		<div class="flex flex-wrap items-center justify-between gap-3">
			<div><h1 class="m-0 text-3xl font-black text-contrast">Members</h1><p class="m-0 text-secondary">Manage invites, roles, bans, and access tiers.</p></div>
		...
		<section class="rounded-2xl bg-surface-3 p-5">
			...
			<div v-for="member in displayMembers" :key="member._id" class="flex flex-wrap items-center justify-between gap-3 rounded-xl p-3 hover:bg-button-bg">
				...
				<div v-if="canAct(member)" class="flex items-center gap-2">
					<select class="rounded-xl border-0 bg-surface-4 px-3 py-2" :value="presetOf(member)" @change="updatePreset(member, ($event.target as HTMLSelectElement).value as SyncedPermissionPreset)">
		...
		<NewModal ref="modal" header="Invite member" max-width="560px">
			<div class="flex flex-col gap-4">
				...
				<div v-for="user in displayResults" :key="user.userId" class="flex items-center justify-between rounded-xl p-2 hover:bg-button-bg">
					<div class="flex items-center gap-3">
