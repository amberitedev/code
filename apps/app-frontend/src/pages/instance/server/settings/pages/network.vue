...
/**
 * Forked verbatim from the hosting server-settings Network page. The full
 * allocations + DNS UI is intentionally retained for later Core support. Today
 * only the Core-backed primary allocation (server port) is populated; the extra
 * allocation query is stubbed to an empty list (Core has no allocation endpoint
 * yet) and the allocation create/edit/delete handlers stay inert for the future.
 */
...
const {
	...
	// Core does not yet expose secondary allocations; return an empty list so the
	// verbatim allocations UI renders with only the Core-backed primary port.
	queryFn: async () => [] as { name: string; port: number }[],
})
