...
const filesTabLink = computed(() => `/instance/${encodeURIComponent(serverId)}/files`)
...
const normalizeKey = (key: string): string => key.replaceAll('-', '_')
const denormalizeKey = (key: string): string => key.replaceAll('_', '-')
...
const { data: propsData } = useQuery({
	queryKey,
	queryFn: () => core.getProperties(serverId),
...
function syncFormFromData() {
	...
	for (const [key, value] of Object.entries(propsData.value)) {
		if (value != null) flat[normalizeKey(key)] = value
	}
...
function buildPatch(): Record<string, string> {
	...
	for (const key of Object.keys(liveProperties.value)) {
		...
		updates[denormalizeKey(key)] = liveProperties.value[key]
	}
	return updates
}

const { mutateAsync: saveProperties, isPending: isUpdating } = useMutation({
	mutationFn: () => core.patchProperties(serverId, buildPatch()),
	onSuccess: async () => {
