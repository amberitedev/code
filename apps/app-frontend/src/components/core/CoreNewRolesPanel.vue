...
const defaultRoleGrants = [
	'invite-members', 'remove-members', 'ban-members', 'manage-roles', 'edit-member-roles',
	'approve-invites', 'manage-instances', 'start-stop-instances', 'restart-instances',
	'manage-mods', 'manage-worlds', 'manage-files', 'manage-backups', 'manage-network',
	'read-console', 'write-console', 'edit-settings', 'view-activity', 'export-data', 'dangerous-actions',
]
const defaultRoles: CoreRole[] = [
	{ id: 'role-admin', name: 'Admin', description: 'Manages members and Core settings.', icon: 'shield', grants_json: JSON.stringify(defaultRoleGrants), created_at: '1970-01-01T00:00:00.000Z', updated_at: '1970-01-01T00:00:00.000Z' },
...
const localRoles = useStorage<CoreRole[]>('amberite:core-new-role-prototype', cloneRoles(defaultRoles))
...
const duplicateOptions = computed(() => roles.value.map((role) => ({ value: role.id, label: role.name })))
...
function roleDraft(role: CoreRole) {
	return { name: role.name, description: role.description, icon: role.icon, grants: parseGrants(role.grants_json) }
}
	...
	<div class="flex min-h-0 flex-1 gap-4">
		<aside class="flex w-64 shrink-0 flex-col gap-2 rounded-xl bg-surface-2 p-3">
			...
			<button
				...
				:class="role.id === selectedId ? 'bg-brand-highlight text-contrast' : 'text-primary hover:bg-surface-3'"
				@click="selectedId = role.id"
			>
				<component :is="icons.find((icon) => icon.id === role.icon)?.component ?? ShieldIcon" class="size-5" />
				<span class="min-w-0 truncate font-semibold">{{ role.name }}</span>
			</button>
			<ButtonStyled color="brand" class="mt-auto">
				<button :disabled="roles.length >= 4" @click="showCreate"><PlusIcon /> Add role ({{ roles.length }} / 4)</button>
			</ButtonStyled>
		</aside>
		<section v-if="selectedRole" class="flex min-w-0 flex-1 flex-col gap-5 rounded-xl bg-surface-2 p-6">
			<div class="flex items-start justify-between gap-4">
				<div class="flex items-center gap-3"><component :is="iconComponent" class="size-8 text-brand" /><div><h2 class="m-0 text-2xl font-semibold text-contrast">{{ selectedRole.name }}</h2><p class="m-0 text-secondary">{{ usingLocalRoles ? 'Saved locally until a Core is linked.' : 'Edit this role and its permissions in one place.' }}</p></div></div>
				<ButtonStyled color="red"><button :disabled="roles.length <= 1" @click="deleteModal?.show()">Delete role</button></ButtonStyled>
			</div>
			<div class="grid gap-3 md:grid-cols-2"><StyledInput v-model="draftName" label="Role name" :maxlength="20" /><StyledInput v-model="draftDescription" label="Description" /></div>
			<div><span class="font-semibold text-contrast">Icon</span><div class="mt-2 flex flex-wrap gap-2"><button v-for="icon in icons" :key="icon.id" class="flex items-center gap-2 rounded-lg border border-solid px-3 py-2" :class="draftIcon === icon.id ? 'border-brand bg-brand-highlight' : 'border-surface-5'" @click="draftIcon = icon.id"><component :is="icon.component" class="size-4" />{{ icon.label }}</button></div></div>
			<div v-for="[group, items] in groupedPermissions" :key="group" class="flex flex-col gap-2"><h3 class="m-0 text-lg font-semibold text-contrast">{{ group }}</h3><div class="grid gap-2 md:grid-cols-2"><label v-for="permission in items" :key="permission.id" class="flex items-center justify-between gap-3 rounded-lg bg-surface-3 px-3 py-2 text-primary"><span>{{ permission.name }}</span><Toggle :model-value="draftGrants.includes(permission.id)" @update:model-value="toggleGrant(permission.id, $event)" /></label></div></div>
			<div class="sticky bottom-0 flex justify-end bg-surface-2 pt-3"><ButtonStyled><button :disabled="isSaving" @click="resetSelected">Reset</button></ButtonStyled><ButtonStyled color="brand"><button :disabled="isSaving || !draftName.trim()" @click="saveSelected">Save role</button></ButtonStyled></div>
		</section>
		<div v-else class="flex flex-1 items-center justify-center text-secondary">{{ isLoading ? 'Loading roles…' : 'No roles available.' }}</div>
		<NewModal ref="createModal" header="Create role" max-width="34rem"><div class="flex flex-col gap-4"><StyledInput v-model="createName" label="Role name" :maxlength="20" /><StyledInput v-model="createDescription" label="Description" /><Combobox v-model="duplicateFrom" :options="duplicateOptions" display-value="Duplicate permissions from" /><div><span class="font-semibold text-contrast">Icon</span><div class="mt-2 flex gap-2"><button v-for="icon in icons" :key="icon.id" class="rounded-lg border border-solid p-2" :class="createIcon === icon.id ? 'border-brand bg-brand-highlight' : 'border-surface-5'" @click="createIcon = icon.id"><component :is="icon.component" class="size-5" /></button></div></div></div><template #actions><div class="flex justify-end gap-2"><ButtonStyled><button @click="createModal?.hide()">Cancel</button></ButtonStyled><ButtonStyled color="brand"><button :disabled="!canCreate" @click="createRole">Create role</button></ButtonStyled></div></template></NewModal>
		<ConfirmModal ref="deleteModal" title="Delete role?" description="Members and pending invites keep this role’s current permissions until an approver assigns a new role. They will be marked as needing reassignment." proceed-label="Delete role" danger @proceed="deleteSelected" />
