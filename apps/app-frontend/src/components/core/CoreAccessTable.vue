...
const emit = defineEmits<{
	updateRole: [member: CoreAccessMember, role: ServerAccessRole]
	resendInvite: [member: CoreAccessMember]
	cancelInvite: [member: CoreAccessMember]
	removeMember: [member: CoreAccessMember]
	inviteMember: [member: CoreAccessMember]
}>()
...
type AccessTableRow = CoreAccessMember & Record<string, unknown>
...
function statusLabel(member: CoreAccessMember): string {
...
function joinedTimestamp(member: CoreAccessMember): number {
...
function resendInviteCooldownSeconds(member: CoreAccessMember): number {
...
function resendInviteCooldownDisabled(member: CoreAccessMember): boolean {
	return resendInviteCooldownSeconds(member) > 0
}

function resendInviteDisabled(member: CoreAccessMember): boolean {
	return !canManageUsers.value || resendInviteCooldownDisabled(member)
}

function resendInviteLabel(member: CoreAccessMember): string {
...
function resendInviteTooltip(member: CoreAccessMember): string {
	return canManageUsers.value ? resendInviteLabel(member) : permissionDeniedMessage.value
}

function handleResendInvite(member: CoreAccessMember) {
	if (resendInviteDisabled(member)) return
	emit('resendInvite', member)
}

function memberAccessActionLabel(member: CoreAccessMember): string {
	return member.pending ? formatMessage(messages.cancelInvite) : formatMessage(messages.removeUser)
}

function memberAccessActionTooltip(member: CoreAccessMember): string {
	return canManageUsers.value ? memberAccessActionLabel(member) : permissionDeniedMessage.value
}

function handleUpdateRole(member: CoreAccessMember, role: ServerAccessRole) {
	if (!canManageUsers.value) return
	emit('updateRole', member, role)
}

function handleCancelInvite(member: CoreAccessMember) {
	if (!canManageUsers.value) return
	emit('cancelInvite', member)
}

function handleRemoveMember(member: CoreAccessMember) {
	if (!canManageUsers.value) return
	emit('removeMember', member)
}

function handleInviteMember(member: CoreAccessMember) {
	if (!canManageUsers.value) return
	emit('inviteMember', member)
}

function memberActionOptions(member: CoreAccessMember): OverflowMenuOption[] {
	return [
		{
			id: 'resend-invite',
			icon: SendIcon,
