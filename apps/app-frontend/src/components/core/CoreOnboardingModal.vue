...
import { ArrowLeftIcon, CheckIcon, ChevronRightIcon, LinkIcon, ServerStackIcon } from '@modrinth/assets'
...
const { fakeUsers, setPreviewState } = useCorePreview()
const setupModalWidth = 'min(928px, calc(95vw - 10rem))'
...
async function pair() {
	...
	try {
		const claimed = await useSocialClient().claimPairingCore(code.value.trim())
		if (!claimed) throw new Error('Pairing code not found or expired.')
		const coreUrl = claimed.connectionUrl ?? (await useCoreClient().adapter.getCoreUrl())
		if (!coreUrl) throw new Error('Core did not publish a connection URL.')
		window.localStorage.setItem(CORE_URL_STORAGE_KEY, coreUrl)
		useCoreClient().clearCoreUrlCache()
		const result = await useCoreClient().completeSetupAt(coreUrl, {
...
async function searchUsers() {
	...
	try {
		const realResults = await useSocialClient().searchUsers(q).catch(() => [])
		const fakeResults = fakeUsers.filter((user) =>
			`${user.username} ${user.displayName} ${user.friendCode}`.toLowerCase().includes(q.toLowerCase()),
		)
		inviteSearchResults.value = [...realResults, ...fakeResults]
	} finally {
...
async function runFinish() {
	...
	if (props.mode === 'setup' && currentUser.value && !coreId.value) {
		...
		await createGroup({ coreId: id, name: groupName.value, setupMode: 'local' }).catch(() => undefined)
		coreId.value = id
...
const stageConfigs = computed<StageConfigInput<unknown>[]>(() => [
	{ id: 'code', title: 'Pairing code', stageContent: CodeStage, skip: isSetup.value, leftButtonConfig: null, rightButtonConfig: {
		...
		disabled: step1Loading.value || !/^\d{6}$/.test(code.value.trim()), loading: step1Loading.value, onClick: pair }, cannotNavigateForward: !coreId.value, maxWidth: setupModalWidth },
	{ id: 'general', title: 'General', stageContent: GeneralStage, leftButtonConfig: isSetup.value ? null : back, rightButtonConfig: { label: 'Continue', icon: ChevronRightIcon, onClick: saveGeneral }, cannotNavigateForward: !groupName.value.trim(), maxWidth: setupModalWidth },
	{ id: 'members', title: 'Members', stageContent: MembersStage, leftButtonConfig: back, rightButtonConfig: { label: 'Continue', icon: ChevronRightIcon, onClick: () => modal.value?.nextStage() }, maxWidth: setupModalWidth },
	{ id: 'advanced', title: 'Advanced', stageContent: AdvancedStage, leftButtonConfig: back, rightButtonConfig: { label: 'Start', icon: ServerStackIcon, onClick: saveAdvanced }, maxWidth: setupModalWidth },
	{ id: 'done', title: isSetup.value ? 'Install Core' : 'Connected', stageContent: DoneStage, nonProgressStage: true, leftButtonConfig: null, rightButtonConfig: { label: 'Finish', icon: CheckIcon, disabled: progress.value < 100, onClick: finish }, maxWidth: setupModalWidth },
])

function show() {
	code.value = ''; step1Error.value = null; step1Loading.value = false; coreId.value = null
	groupName.value = 'Amberite Core'; description.value = ''; iconUrl.value = ''; bannerUrl.value = ''
	subdomain.value = ''; invitedUsers.value = []; generatedInviteCode.value = null
	progress.value = 0; progressLabel.value = 'Waiting to start'
