...
import { autoPairLocalDevelopmentCore } from '@/core/auto-pair-local-core'
...
async function fetchCredentials() {
	...
	if (session) {
		startPresence()
		await autoPairLocalDevelopmentCore(session.user).catch(handleError)
...
<template>
	...
	>
		...
		>
			...
			<TeleportOverflowMenu
				...
				:tooltip="formatMessage(messages.modrinthAccount)"
				type="quiet"
				size="xl"
	...
	>
		...
		>
			<div
				class="app-sidebar-scrollable min-h-0 flex-grow shrink overflow-x-hidden overflow-y-auto relative"
			>
				<OnboardingChecklist
				...
				<div class="sidebar-default-content" :class="{ 'sidebar-enabled': sidebarVisible }">
					...
					<Accordion
						...
						open-by-default
						button-class="flex w-full cursor-pointer border-0 bg-transparent p-0"
						class="p-4"
					>
						<template #title>
							<h3 class="m-0 text-left text-base font-medium text-primary">
