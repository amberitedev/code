<script setup>
import { AuthFeature, PanelVersionFeature, GenericModrinthClient } from '@modrinth/api-client'
import {
	ArrowBigUpDashIcon,
	ChangeSkinIcon,
	CompassIcon,
...
import { invoke, openUrl, type } from '@tauri-apps/api/core'
...
const apiClient = new GenericModrinthClient({
	userAgent: `modrinth/app-web/1.0.0 (support@modrinth.com)`,
	features: [
		new AuthFeature({
...
async function setupApp() {
	...
	if (os.value !== 'MacOS') {
		try {
			...
			// Web mode - no window decorations
		}
	}
	...
	try {
		...
		// Web mode - no window state tracking
	}
...
const handleClose = async () => {
	try {
		...
		console.log('Cannot close window in web mode')
	}
}
...
onMounted(() => {
	// Web mode - no Tauri window to show
	console.log('App mounted in web mode')
...
async function checkUpdates() {
	...
	async function performCheck() {
		// Web mode - no Tauri updater
		console.log('Update checking disabled in web mode')
		return null
	}
...
<template>
	<SplashScreen v-if="!stateFailed" ref="splashScreen" />
	...
	>
		<Transition name="fade">
			...
			>
				...
				>
					<RefreshCwIcon class="animate-spin w-6 h-6" />
					Restarting...
				</span>
			</div>
		</Transition>
		...
		>
			...
			<NavButton
				v-tooltip.right="'Brand Editor (experimental)'"
				to="/brand-editor"
			>
		...
		<div class="app-grid-statusbar bg-bg-raised h-[--top-bar-height] flex">
