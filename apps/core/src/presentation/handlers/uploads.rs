...
pub async fn create_upload_handler(
    ...
    let instance_id =
        resolve_upload_instance_id(&state, &claims.sub, &id).await?;
...
pub async fn upload_status_handler(
    ...
    let instance_id =
        resolve_upload_instance_id(&state, &claims.sub, &id).await?;
    let session = upload_status(&state, &instance_id, &upload_id)
        .map_err(upload_error)?;
...
async fn resolve_upload_instance_id(
    ...
    Ok(
        resolve_authorized_instance_id(state, user_id, path, "server:files")
            .await?
            .to_string(),
    )
}
