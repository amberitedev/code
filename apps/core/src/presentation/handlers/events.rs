...
use crate::{
    ...
    domain::event::Event,
    presentation::{
        authz::{require_core_member, require_instance_permission},
        error::ApiError,
        extractors::AuthUser,
    },
};
        ...
        .filter_map(move |msg| {
            ...
            async move {
                let event = match msg.ok()? {
                    ...
                    | event @ Event::InstanceUpdated { .. }
                    | event @ Event::InstanceDeleted { .. }
                    ...
                    | event @ Event::SyncProfileUpdated { .. } => {
                        let instance_id = match &event {
                            ...
                            | Event::StatusChanged { instance_id, .. }
                            | Event::CreationProgress { instance_id, .. }
