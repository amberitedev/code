...
use crate::{
	application::{
		fs_service::{
			copy_files, create_dir, create_file, delete_entry, download_file, get_download_url,
			list_directory, move_entry, read_file, search_files, unzip_file, upload_file,
			write_file, zip_files, FsError, UnzipOption,
		},
		state::AppState,
	},
...
impl From<FsError> for ApiError {
	fn from(e: FsError) -> Self {
		match e {
			...
			FsError::NotAFile => ApiError::BadRequest("path is a directory, not a file".into()),
...
pub async fn read_handler(
	...
	validate_id(&id)?;
	let bytes = read_file(&state, &id, &q.path).await.map_err(ApiError::from)?;
...
pub async fn write_handler(
	...
	validate_id(&id)?;
	write_file(&state, &id, &q.path, body).await.map_err(ApiError::from)?;
	Ok(Json(json!({ "ok": true })))
}
...
pub async fn create_file_handler(
	...
	validate_id(&id)?;
	create_file(&state, &id, &q.path).await.map_err(ApiError::from)?;
	Ok(Json(json!({ "ok": true })))
}
...
pub async fn mkdir_handler(
	...
	validate_id(&id)?;
	create_dir(&state, &id, &q.path).await.map_err(ApiError::from)?;
	Ok(Json(json!({ "ok": true })))
}
...
pub async fn move_handler(
	...
	validate_id(&id)?;
	move_entry(&state, &id, &q.from, &q.to).await.map_err(ApiError::from)?;
	Ok(Json(json!({ "ok": true })))
}
...
pub async fn unzip_handler(
	...
	validate_id(&id)?;
	unzip_file(&state, &id, &q.path, body.option.unwrap_or(UnzipOption::Normal))
		.await
		.map_err(ApiError::from)?;
	Ok(Json(json!({ "ok": true })))
}
...
pub async fn zip_handler(
	...
	validate_id(&id)?;
	zip_files(&state, &id, body.sources, &body.dest).await.map_err(ApiError::from)?;
	Ok(Json(json!({ "ok": true })))
}
...
pub async fn copy_handler(
	...
	validate_id(&id)?;
	copy_files(&state, &id, body.sources, &body.dest).await.map_err(ApiError::from)?;
	Ok(Json(json!({ "ok": true })))
}
...
pub async fn url_handler(
	...
	validate_id(&id)?;
	let resp = get_download_url(&state, &id, &q.path).await.map_err(ApiError::from)?;
	Ok(Json(json!(resp)))
}
...
pub async fn search_handler(
	...
	let results = search_files(&state, &id, base, &q.query, q.recursive.unwrap_or(false))
		.await
		.map_err(ApiError::from)?;
...
pub async fn download_by_key_handler(
	...
	let token = state
		...
		.remove(&key)
		.map(|(_, t)| t)
		.ok_or_else(|| ApiError::NotFound("download token not found or expired".into()))?;
