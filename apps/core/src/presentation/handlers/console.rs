...
use crate::{
	application::{instance_status_service::send_command, state::{AppState, WsTicket}},
...
async fn collect_stats_for_ws(state: &Arc<AppState>, iid: &InstanceId) -> Value {
	let (uptime_seconds, pid) = match state.instances.get(iid) {
		Some(h) => (Some(h.started_at.elapsed().as_secs()), h.pid),
		None => return json!({ "cpu_percent": null, "memory_mb": null, "ram_total_mb": null, "player_count": null, "uptime_seconds": null }),
	};
	...
	let (cpu_percent, memory_mb, ram_total_mb) = if let Some(pid_val) = pid {
		tokio::task::spawn_blocking(move || {
			...
			sys.refresh_all();
			sys.process(p)
				.map(|proc| (Some(proc.cpu_usage()), Some(proc.memory() / 1_048_576), Some(sys.total_memory() / 1_048_576)))
