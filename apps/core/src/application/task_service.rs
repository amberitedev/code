...
impl ScheduledTask {
	pub fn payload_json(&self) -> Option<Value> {
		self.payload.as_ref().and_then(|p| serde_json::from_str(p).ok())
	}
}
...
pub async fn list_tasks(
	...
	let _ = instance_id.parse::<InstanceId>().map_err(|_| TaskError::NotFound)?;
...
pub async fn delete_task(
	...
	let result = sqlx::query("DELETE FROM scheduled_tasks WHERE id = ? AND instance_id = ?")
		.bind(task_id)
		.bind(instance_id)
		.execute(&state.pool)
		.await?;
...
pub async fn list_all_enabled_tasks(state: &Arc<AppState>) -> Result<Vec<ScheduledTask>, TaskError> {
	let rows: Vec<ScheduledTask> = sqlx::query_as::<_, ScheduledTask>(
		"SELECT id, instance_id, task_type, cron, enabled, payload, created_at, updated_at, last_run_at
