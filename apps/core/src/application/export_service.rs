...
pub async fn export_modpack(
    ...
    for row in &mod_rows {
        ...
        if let (Some(pid), Some(sha)) = (pid, sha) {
            ...
            private_mods
                .push((filename.clone(), data_dir.join("mods").join(filename)));
        }
    }
    ...
    let zip_bytes =
        tokio::task::spawn_blocking(move || -> Result<Vec<u8>, ExportError> {
            ...
            if config_dir.exists() {
                for entry in
                    walkdir::WalkDir::new(&config_dir).into_iter().flatten()
                {
                    ...
                    if let Ok(rel) = entry.path().strip_prefix(&config_dir) {
                        ...
                        zip.start_file(
                            format!("overrides/config/{rel_str}"),
                            opts,
                        )?;
    ...
    let filename =
        format!("{}.mrpack", sanitize_filename::sanitize(&pack_name));
