...
fn emit(state: &Arc<AppState>, instance_id: &str, operation: FsOperationKind, path: &Path) {
	let id = instance_id.parse::<InstanceId>().unwrap_or_else(|_| InstanceId(Uuid::nil()));
	let path_str = path.to_string_lossy().replace('\\', "/");
	state.broadcaster.send(Event::FsChanged { instance_id: id, operation, path: path_str });
}
...
pub async fn move_entry(
	...
	// Destination may not exist yet — only guard parent.
	let to_parent = to.parent().unwrap_or(&data_dir);
	let _ = guard_path(&data_dir, to_parent.to_string_lossy().as_ref())?;
	...
	emit(
		state,
		instance_id,
		FsOperationKind::Move { from: from_path.to_string() },
		&to,
	);
	Ok(())
}
...
pub async fn unzip_file(
	...
	tokio::task::spawn_blocking(move || {
		...
		std::fs::create_dir_all(&effective_dest).map_err(zip::result::ZipError::Io)?;

		for i in 0..archive.len() {
			let mut entry = archive.by_index(i)?;
			let entry_path = effective_dest.join(entry.name());
			if entry.is_dir() {
				std::fs::create_dir_all(&entry_path).map_err(zip::result::ZipError::Io)?;
			} else {
				if let Some(p) = entry_path.parent() {
					std::fs::create_dir_all(p).map_err(zip::result::ZipError::Io)?;
				}
				let mut out = std::fs::File::create(&entry_path).map_err(zip::result::ZipError::Io)?;
		...
		for src in &canonical_sources {
			if src.is_dir() {
				for entry in WalkDir::new(src).into_iter().filter_map(|e| e.ok()) {
					...
					let name = entry_path.strip_prefix(src.parent().unwrap_or(src)).unwrap_or(entry_path);
					let name_str = name.to_string_lossy().replace('\\', "/");
					if entry_path.is_dir() {
						zip.add_directory(&name_str, options).map_err(|e| FsError::Zip(e))?;
					} else {
						zip.start_file(&name_str, options).map_err(|e| FsError::Zip(e))?;
				...
				zip.start_file(name.as_ref(), options).map_err(|e| FsError::Zip(e))?;
...
pub async fn get_download_url(
	...
	state.fs_download_tokens.insert(key.clone(), FsDownloadToken {
		...
		for entry in WalkDir::new(&base).into_iter().filter_map(|e| e.ok()) {
			...
			if name.contains(&query_lower) {
				if let Ok(meta) = entry.metadata() {
					results.push(entry_to_json(&data_dir, entry.path(), meta.is_dir(), &meta));
				}
			}
		}
	} else {
		...
		while let Some(e) = rd.next_entry().await? {
			...
			if name.contains(&query_lower) {
				if let Ok(meta) = e.metadata().await {
					results.push(entry_to_json(&data_dir, &e.path(), meta.is_dir(), &meta));
				}
			}
		}
	}
