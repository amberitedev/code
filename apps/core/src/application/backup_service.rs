...
pub enum BackupError {
	...
	#[error("backup locked")] Locked,
	#[error("instance must be offline to restore")] MustBeOffline,
}

async fn data_dir_for(state: &Arc<AppState>, instance_id: &str) -> Result<PathBuf, BackupError> {
...
fn zip_data_dir(data_dir: &Path, zip_path: &Path) -> Result<u64, BackupError> {
	...
	for entry in walkdir::WalkDir::new(data_dir).min_depth(1) {
		let entry = entry.map_err(|e| {
			let msg = e.to_string();
			BackupError::Io(e.into_io_error().unwrap_or_else(|| std::io::Error::other(msg)))
		})?;
		let path = entry.path();
		...
		if path.is_dir() {
			zip.add_directory(&name, options).map_err(|e| BackupError::Zip(e.to_string()))?;
		} else {
			zip.start_file(&name, options).map_err(|e| BackupError::Zip(e.to_string()))?;
...
fn unzip_to(zip_path: &Path, target: &Path) -> Result<(), BackupError> {
	...
	let mut archive = zip::ZipArchive::new(file).map_err(|e| BackupError::Zip(e.to_string()))?;
	for i in 0..archive.len() {
		let mut entry = archive.by_index(i).map_err(|e| BackupError::Zip(e.to_string()))?;
		let out = target.join(entry.name());
		if entry.is_dir() {
...
pub async fn create_backup(
	...
	let display_name = name.unwrap_or_else(|| {
		chrono::Utc::now().format("backup-%Y%m%d-%H%M%S").to_string()
	});
...
pub async fn delete_backup(
	...
	let row: Option<(bool,)> =
		sqlx::query_as("SELECT locked FROM backups WHERE id = ? AND instance_id = ?")
			.bind(backup_id)
			.bind(instance_id)
			.fetch_optional(&state.pool)
			.await?;
	...
	sqlx::query("DELETE FROM backups WHERE id = ?").bind(backup_id).execute(&state.pool).await?;
	Ok(())
}
...
pub async fn lock_backup(
	...
	let rows = sqlx::query("UPDATE backups SET locked = ? WHERE id = ? AND instance_id = ?")
		.bind(locked)
		.bind(backup_id)
		.bind(instance_id)
		.execute(&state.pool)
...
pub async fn restore_backup(
	...
	let iid: InstanceId = instance_id.parse().map_err(|_| BackupError::NotFound)?;
	...
	let _ = create_backup(state, instance_id, "manual", Some("pre-restore".to_string())).await;
...
pub async fn get_backup_schedule(
	...
	.fetch_optional(&state.pool)
	.await?;
	Ok(row.map(|(e, c, r)| BackupSchedule { enabled: e, cron: c, retain_count: r })
		.unwrap_or(BackupSchedule { enabled: false, cron: "0 4 * * *".to_string(), retain_count: 5 }))
}
...
pub async fn rename_backup(
	...
	let rows = sqlx::query("UPDATE backups SET name = ? WHERE id = ? AND instance_id = ?")
		.bind(&name)
		.bind(backup_id)
		.bind(instance_id)
		.execute(&state.pool)
