...
use crate::application::{backup_service::{create_backup, storage_dir}, state::AppState};
...
async fn tick(state: &Arc<AppState>) -> Result<(), sqlx::Error> {
	...
	for (instance_id, cron, retain_count) in schedules {
		if !cron_matches(&cron, now.minute(), now.hour(), now.day(), now.month(), now.weekday().num_days_from_sunday()) {
			continue;
		}
		if let Err(e) = create_backup(state, &instance_id, "automated", None).await {
...
fn cron_matches(cron: &str, minute: u32, hour: u32, dom: u32, month: u32, dow: u32) -> bool {
