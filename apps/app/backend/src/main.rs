...
use infrastructure::supabase_auth::SupabaseJwtValidator;
...
async fn main() -> Result<()> {
    ...
    let jwt_secret = std::env::var("SUPABASE_JWT_SECRET")
        .expect("SUPABASE_JWT_SECRET must be set in .env");
    let jwt_validator = SupabaseJwtValidator::new(&jwt_secret);
    ...
    let router = create_router(registry, jwt_validator);
