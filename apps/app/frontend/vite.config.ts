...
// Import from vite-plugin.ts directly instead of barrel index to avoid
// loading vue-plugin.ts which imports .vue files (chokes Node.js during Vite config loading)
import { designInspectorVitePlugin } from '@amberite/design-inspector/vite-plugin.ts'

import tauriConf from '../tauri.conf.json'

const projectRootDir = resolve(__dirname)
const appLibEnvDir = resolve(projectRootDir, '..')

// Load .env from app directory manually instead of using Vite's envDir, which would auto-load .env.local and override values
...
export default defineConfig({
	...
	resolve: {
		alias: [
			...
			// Fix relative imports from app/frontend to workspace packages at root
			// From app/frontend/src/components/ui/world: need to go up 7 levels to reach packages
			{
				find: '../../../../../../packages/',
				replacement: resolve(projectRootDir, '../../../../../packages/'),
			},
		],
	},
	plugins: [
		...
		...(process.env.NODE_ENV !== 'production' ? designInspectorVitePlugin() : []),
	],
